...
// Cache is a struct in which the cache's methods synchronize access to this map,
// so it is not recommended to keep any references to the map around after creating a cache.
type Cache struct {
	*cache
}
...
// If the expiration duration is less than one, the items in the cache never expire (by default),
